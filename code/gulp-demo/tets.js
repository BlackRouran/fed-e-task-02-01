...
const cwd = process.cwd() // 返回命令行的工作目录
...
const useref = () => {
  return src(config.build.paths.pages, { base: config.build.dist, cwd: config.build.temp })
  ...
  bs.init({
    ...
    server: {
      baseDir: [ //请求过来后 依次从数组中去找
        config.build.temp, config.build.dist, config.build.public
      ],
      routes: {
